...
[The Fluxible R package](https://plant-functional-trait-course.github.io/fluxible/index.html) (@fluxible-dev) is designed to process ecosystem gas fluxes measurements from raw concentration over time measurements into a clean dataset of gas fluxes in a reproducible and automated method.
...
But the dataset provided in this exercise highlighted some issues.
They were fixed in the devellopment version.
We therefore recommend to install the devellopment version for this exercise.
...
```{r install-dev, warning = FALSE, message = FALSE}
...
## Issues that were fixed in the devellopment version
...
# str(ch4_slopes_exp_flag)

ch4_slopes_qua_flag <- flux_quality(ch4_slopes_qua,
                                    ambient_conc = 3,
                                    error = 5)

# str(ch4_slopes_qua_flag)

ch4_slopes_lin_flag <- flux_quality(ch4_slopes_lin,
                                    ambient_conc = 3,
                                    error = 5)

# str(ch4_slopes_lin_flag)
```

After adding the quality flags we can plot the fluxes to visually check them.

```{r ch4_plot, fig.width = 8, fig.height = 9}
ch4_slopes_exp_flag  |>
...
It seems that the slopes still describe the flux well despite low R^2.
...
```{r re_eval_ch4, fig.width = 8, fig.height = 9}
ch4_slopes_qua_flag <- flux_quality(ch4_slopes_qua,
                                    ambient_conc = 3,
                                    error = 5,
                                    force_ok_id = c(5, 6),
...
# str(n2o_slopes_exp_flag)

n2o_slopes_qua_flag <- flux_quality(n2o_slopes_qua,
                                    ambient_conc = 0.325,
                                    error = 0.05)

# str(n2o_slopes_qua_flag)

n2o_slopes_lin_flag <- flux_quality(n2o_slopes_lin,
                                    ambient_conc = 0.325,
                                    error = 0.05)

# str(n2o_slopes_lin_flag)
```

After adding the quality flags we can plot the fluxes to visually check them.

```{r n2o_plot, fig.width = 8, fig.height = 9, warning = FALSE, message = FALSE}
n2o_slopes_exp_flag  |>
...
I will just the linear fit, effectively replacing all slopes by zero.

## Calculating the fluxes

```{r n2o_calc}
n2o_fluxes_lin <- flux_calc(n2o_slopes_lin_flag,
                            slope_col = "f_slope_corr",
                            conc_unit = "ppb",
                            flux_unit = "micromol",
                            chamber_volume = "Vtot",
...
```{r merge_all, warning = FALSE, message = FALSE}
all_fluxes <- bind_rows(co2_fluxes_exp, ch4_fluxes_qua, n2o_fluxes_lin)
